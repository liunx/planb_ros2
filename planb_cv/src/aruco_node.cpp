#include <thread>
#include "planb_cv/aruco_node.hpp"
#include "planb_interfaces/msg/bounding_box.hpp"
      ...
      status_("OFF")
{
    ...
    pub_data_  = this->create_publisher<planb_interfaces::msg::ArucoMarker>("/planb/aruco/data", 1);
...
void ArucoNode::stream_callback(const sensor_msgs::msg::Image::UniquePtr msg)
{
    ...
    if (ids.size() > 0) {
        planb_interfaces::msg::ArucoMarker _msg;
        _msg.header.stamp = this->get_clock()->now();
        for (size_t i{0}; i < ids.size(); i++) {
            planb_interfaces::msg::BoundingBox bbox;
            auto _id = ids.at(i);
...
void ArucoNode::cmd_callback(const planb_interfaces::msg::Cmd &msg)
{
